...
es que las características de los datos han sido bien escogidas y formadas. % Características "¿formadas?" - JJ
...
confianza, como hemos visto en~\ref{cap:resultados}. %¿Qué es "alta"?
                                %¿Qué nivel decidiste de antemano? - JJ
...
y~\citep{DBLP:conf/aiide/StanescuHEGB13}. % Este es el otro objetivo. 
% Tendrías que decir algo sobre cómo las procesado los datos. ¿Usas
% una ventana que corre? ¿Agregados temporales? ¿Sólo el último
% instante? - JJ
% También tendrías que decir por qué uno solo no cubre los dos
% objetivos y si es un problema que haya dos. 
...
dan resultados cercanos. % ¿Este es de ese tipo de problemas? ¿Qué
                         % implica eso con respecto al problema? - JJ
El instante \emph{Frame} es un buen ejemplo: en dos
...
desarrolladas. Como se ha comentado en~\ref{cap:resultados}, con sólo
10 minutos % ¿10 minutos es un 1% o un 90%? - JJ
...
metaheurísticas apropiadas sería más rápida. No sólo es interesante % no es "el problema planteado". Es uno de los objetivos - JJ
