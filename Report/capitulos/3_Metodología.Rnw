...
que se ha extraído siguiendo los pasos de las
secciones~\ref{sec:seleccion,sec:preprocesamiento}, los separa en entrenamiento
y test, forma un \emph{Pipeline}
cuya salida es un modelo aprendido, forma un grid de parámetros para el modelo
en cuestión y lo tunea utilizando Validación Cruzada con 10 particiones.
Por último, guarda el modelo resultante en disco.
...
Éste, llamado \emph{Testing}, carga los modelos guardados en disco y calcula
la precisión, el \emph{AUC} y la medida \emph{F1} en el conjunto de test que
previamente se había separado.
