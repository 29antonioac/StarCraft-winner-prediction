...
Existe algún trabajo de predicción utilizando aprendizaje supervisado,
...
\citep{adaptativeStrategyPrediction} o en \citep{makingAndActing}. % Un estado del arte no es una enumeración de trabajos que hagan cosas similares. Es una muestra de cómo y en qué líneas de investigación ha avanzado el campo, del cual se extrae un "estado del arte" que hay que superar - JJ
